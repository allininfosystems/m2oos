...
class Signup extends Action
{
    ...
    public function execute()
    {
		...
		$product_id_post = $this->getRequest()->getPost('product_id');
		
		if($product_id_post){
			$msgArr = array();
			if ($this->isAlreadySubscribed() === false) {
				$this->saveStockNotification();
				$msgArr['msg'] = 'We will send an email to '.$email.' when the product will back in stock!';
				$msgArr['flag'] = '1'; 
				$this->getPostEmail();
				
			} else {
				$msgArr['msg'] = 'Your e-mail '.$email.' is already on the list!';
				$msgArr['flag'] = '2'; 
				$this->getPostEmail();
			}

			$result = json_encode($msgArr);
